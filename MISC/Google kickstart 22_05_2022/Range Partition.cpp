/*
Problem
Alan and Barbara suddenly felt like playing with numbers. Alan chooses a non-empty subset from the set of first N positive integers (1,2,…,N). 
Barbara takes the rest of the numbers (if any) from the set. And then they both calculate the sum of the elements in their respective sets.

Alan believes in a magic ratio, which is X:Y. Hence, Alan wants to choose the subset in such a way that the ratio between the sum of Alan's 
subset and the sum of Barbara's subset is exactly X:Y.
...
Output
For each test case, output the first line containing Case #x: y, where x is the test case number (starting from 1) and y is POSSIBLE, if 
Alan can choose such a non-empty subset, and IMPOSSIBLE otherwise.
...
int main()
{
    ...
    for(int i=1;i<=testCase;i++)
    {
        ...
        bool ans = true;
        if((totalSum)%(x+y)!=0) ans = false;//Not necessary to check (totalSum*x)%(x+y)
        if(ans)
		{
			cout<<"Case #"<<i<<": POSSIBLE";
			vector<int> nums;
			int sum = (totalSum/(x+y))*x;//first divide then multiply otherwise overflow will happen
			while(sum>0)
			{
				if(n<=sum)
				{
