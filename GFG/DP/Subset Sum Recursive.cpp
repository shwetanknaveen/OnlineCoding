...
public:
    ...
    int recursiveSubsetSum(int N,int arr[],int sum,vector<vector<int>> &dp,int ind)
    {
        ...
        dp[ind][sum] = (recursiveSubsetSum(N,arr,sum-arr[ind],dp,ind+1)) ==1 || (recursiveSubsetSum(N,arr,sum,dp,ind+1) == 1)? 1 : 0;//if it was possible to get the sum either way, with or without choosing the element then we put 1(possible)
        			 /*We choose the element and subtract its weight		We leave the element and sum as it is*/
        else
        dp[ind][sum] = recursiveSubsetSum(N,arr,sum,dp,ind+1);//if current element under consideration has weight > required sum
    ...
    bool isSubsetSum(int N, int arr[], int sum){
        ...
        return (recursiveSubsetSum(N,arr,sum,dp,0) == 1 ? true : false);
    }
