...
//Video link - https://www.youtube.com/watch?v=VDhRg-ZJTuc&list=PL_z_8CaSLPWekqhdCPmFohncHwz8TY2Go&index=28

/*Here when we fill the dp matrix using top down approach we assume that i and j are starting indices in string A and B
Hence maximum value of dp is when we start from beginning of both strings i.e., at dp[0][0].
*/
...
public:
    ...
    */
    string shortestCommonSupersequence(string str1, string str2) {
        ...
        int LCSlen = LCS(str1,str2,l1,l2,0,0,dp);
        string SCSstring;
        int i=0,j=0;
        while(i<l1 && j<l2)//any of i and j can't exceed string boundaries
        {
            if(str1[i]==str2[j])
            {
            ...
            else
            {
                ...
                {
                    SCSstring.push_back(str1[i]);//We are about to move down hence we will miss one character from first 
                    							//string so we have to push it in our ans first
                    i++;
                }
                else
                {
                    SCSstring.push_back(str2[j]);//We are about to move right hence we will miss one character from second 
                    							//string so we have to push it in our ans first
                    j++;	//moving towards max cell
                }
            }
        }
        while(i<l1)//pushing the remaining part of string 1
            SCSstring.push_back(str1[i++]);
        while(j<l2)//pushing the remaining part of string 2
