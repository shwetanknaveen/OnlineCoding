        ...
        while(!q.empty()){
            ...
            if(grid[x][y]==1){
                if(t[3]>0) // If we encounter an obstacle and we can remove it
                    ...
                    continue;//We can't remove this obstacle hence 
            }
			
			// The cell was previously visited by some path and we were able to remove less obstacles in the previous path to reach this cell.
			// Then we don't need to continue on our current path as we could reach this cell with removing less number of obstacles
            if(vis[x][y]>=t[3])//if t[3] is less it means that we have removed more number of obstacles in this path
                continue;
            vis[x][y]=t[3];//Now at 
